...
// Class for kill lord randomly event
class KillLord {
...
(async function mainFunc() {
  ...
  await Promise.all(
    shuffle(houseIdList).map((elm) => {
      createDom(elm);
    }),
  );
})();

// Google search keywords: javascript change array position randomly
// Reference link: https://stackoverflow.com/questions/2450954/how-to-randomize-shuffle-a-javascript-array
...
// Button click event
const killRandomLord = new KillLord();
document
  .getElementById('kill-random-lord-button')
  .addEventListener('click', killRandomLord);
